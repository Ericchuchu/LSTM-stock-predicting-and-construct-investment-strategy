...
from data_preprocess import data_preprocess, TrainDataset
...
if __name__ == "__main__" :
    ...
    for stock in selected_stock:
        print(f'{stock} training the model')
        data_train_valid = data_preprocess(f"data/{stock} 2016-2022.csv",'train')
        test = data_preprocess(f"data/{stock} 2023.csv", 'test')
        train = data_train_valid.iloc[:int(len(data_train_valid)*0.8),:] 
        valid = data_train_valid.iloc[int(len(data_train_valid)*0.8):,:] 
        ...
        for epoch in range(args.epochs):
            ...
            if valid_loss_mse < best_loss_mse:
                torch.save(model.state_dict(),f'model_state\{args.model}_{stock}_state_predict_sequence.pth')
