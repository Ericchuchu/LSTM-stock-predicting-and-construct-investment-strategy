...
class ConvLSTMCell(nn.Module):
    ...
    def init_hidden(self, batch_size, hidden, dim, device):
        if self.Wci is None:
            self.Wci = nn.Parameter(torch.zeros(1, hidden, dim)).to(device)
            self.Wcf = nn.Parameter(torch.zeros(1, hidden, dim)).to(device)
            self.Wco = nn.Parameter(torch.zeros(1, hidden, dim)).to(device)
        else:
            assert dim == self.Wci.size()[2], 'Input Dim Mismatched!'
        return (Variable(torch.zeros(batch_size, hidden, dim)).to(device),
                Variable(torch.zeros(batch_size, hidden, dim)).to(device))


class ConvLSTM(nn.Module):
    # input_channels corresponds to the first input feature map
    # hidden state is a list of succeeding lstm layers.
    def __init__(self, input_channels, hidden_channels, kernel_size, in_dim, out_dim, device, step=10):
...
if __name__ == '__main__':
    ...
    model = ConvLSTM(input_channels=3, hidden_channels=[3, 1], kernel_size=3, in_dim=8, out_dim=1, device = device, step=5)
    print(model(x).shape)